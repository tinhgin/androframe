...
class Elsim(object):
    ...
    def show_similar_class_name(self, i, details=True):
        classes = Set([])
        for j in self.filters[ SIMILARITY_SORT_ELEMENTS ][ i ]:
                    classes.add(j.get_class_name())
        return classes

    def show_new_class_name(self, i, details=False):
        return i.get_class_name()


    def show_similar_method_name(self, i, details=True):
        classes = Set([])
        for j in self.filters[ SIMILARITY_SORT_ELEMENTS ][ i ]:
                    classes.add(j.get_method_name())
        return classes

    def show_new_method_name(self, i, details=False):
        return i.get_method_name()
